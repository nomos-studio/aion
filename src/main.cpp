...
int main(int argc, char* argv[]) {
    ...
    uint32_t     buffer_frames   = 64; // small: hardware interrupt cadence, not processing
    bool         no_audio        = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--socket" && i + 1 < argc)
            ...
            std::cout
                << "Usage: aion [options]\n"
                   "  --socket <path>         Unix domain socket (default: /tmp/aion.sock)\n"
                   ...
                   "  --buffer-frames <n>     Audio buffer size for clock discipline (default: 64)\n"
                   "  --audio-device <id>     RtAudio device id (0=default, see "
                   "--list-audio-devices)\n"
                   "  --no-audio              Skip audio device (no hardware clock discipline)\n"
    ...
    // Shared queues.
    // param_queue: written by rt_control_thread on msg_param_set; no audio consumer
    ...
    // Control thread — IPC socket, session, and common cljseq-rt message dispatch.
    kairos::rt_control_thread ctrl{
        kairos::rt_control_thread::config{.socket_path = socket_path, .db_path = db_path},
        param_queue,
        ipc_in_queue};
    ctrl.start();
    ...
    // Audio device — opened at a small buffer size purely for the hardware interrupt
    // cadence that disciplines our clock.  When a wordclock-capable interface is
    // present (e.g. ES-9, Scarlett) this becomes the most stable timestamp source
    // in the room and aion can assert Link authority.  The callback does no audio
    // processing — it just clocks the hardware interrupt.
    ...
    if (!no_audio) {
        ...
        const bool opened = audio_dev.open(
            audio_cfg,
            [](float** out, const float* const*, uint32_t out_ch, uint32_t, uint32_t nframes,
               double) {
    ...
    std::thread event_thread{[&]() {
        auto dispatch_event = [&](const kairos::clap_event_union& ev) {
            ...
            case CLAP_EVENT_NOTE_ON: {
                const auto vel =
                    static_cast<uint8_t>(std::clamp(ev.note.velocity * 127.0, 0.0, 127.0));
