...
class RoutingMatrix {
  public:
    ...
    void apply_edn(std::string_view edn) {
        ...
        if (!parsed || !parsed->is<edn::map>()) return;
        ...
        if (const auto* mr = m.find_kw("mod-routes"); mr && mr->is<edn::vector>()) {
            for (const auto& entry : mr->get<edn::vector>().items) {
                ...
                if (!id_v || !id_v->is<std::string>()) continue;
                ModRoute r;
  ...
  private:
    ...
    static int note_channel(const nomos::rt::clap_event_union& ev) {
        switch (ev.header.type) {
            ...
            case CLAP_EVENT_NOTE_OFF:  return ev.note.channel;
            case CLAP_EVENT_MIDI:      return ev.midi.data[0] & 0x0F;
            default:                   return -1;
        }
    }
    ...
    static MidiRoute::Source parse_source(const edn::map& m) {
        ...
        if (!v || !v->is<edn::keyword>()) return MidiRoute::Source::ipc;
        const auto& kw = v->get<edn::keyword>();
        if (kw == edn::keyword{"midi-hw"}) return MidiRoute::Source::midi_hw;
        if (kw == edn::keyword{"osc"})     return MidiRoute::Source::osc;
    ...
    static int parse_field(const edn::map& m) {
        ...
        if (v->is<edn::keyword>()) {
            const auto& kw = v->get<edn::keyword>();
            if (kw == edn::keyword{"cv"})    return 0;
            if (kw == edn::keyword{"aux"})   return 1;
            if (kw == edn::keyword{"gate"})  return 2;
            if (kw == edn::keyword{"gate2"}) return 3;
            // :out0 .. :out15 — keyword name is e.g. "out0", "out7"
            if (kw.name.size() > 3 && kw.name.substr(0, 3) == "out") {
                try { return 4 + std::stoi(std::string(kw.name.substr(3))); }
                catch (...) {}
            }
        }
        return 0;
    ...
    static int int_field(const edn::map& m, const char* key, int def) {
        ...
        if (!v) return def;
        if (v->is<int64_t>()) return static_cast<int>(v->get<int64_t>());
        return def;
    }
